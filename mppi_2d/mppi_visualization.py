...
def plot_mppi_position_3d(global_ref, traj, walls, N, K):
...
# Plot velocity & acceleration vs distance
def plot_vel_and_acc_and_clearance_along_path(distance, velocity_xyz, acceleration_xyz, clearance):
    plt.figure(figsize=(8, 5))
    plt.plot(distance, velocity_xyz, linewidth=1, color="tab:blue", label="Velocity[m/s]")
    plt.plot(distance, np.full_like(distance, np.mean(velocity_xyz)),
             linewidth=3, linestyle="--", color="tab:blue", label="Mean velocity[m/s]")
    plt.plot(distance, acceleration_xyz, linewidth=1, color="tab:red", label="Acceleration[m/s²]")
    plt.plot(distance, np.full_like(distance, np.max(acceleration_xyz)),
             linewidth=3, linestyle="--", color="tab:red", label="Max acceleration[m/s²]")
    plt.plot(distance, clearance, linewidth=1, color="tab:green", label="clearance[m]")    
    plt.plot(distance, np.full_like(distance, np.min(clearance)),
             linewidth=3, linestyle="--", color="tab:green", label="Min clearance[m]")    
    plt.xlabel("Path length [m]")
    plt.ylabel("Magnitude")
    plt.legend()
    plt.title("Velocity, Acceleration and Clearance profile along trajectory")
    plt.grid(True)
    plt.tight_layout()
    plt.show()
