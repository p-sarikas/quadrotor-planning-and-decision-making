...
def obstacle_penalty(p_xy, collision_fn):
    """
    Generic obstacle penalty using an externally provided collision function
    """
    return 0.0 if collision_fn(p_xy) else 1e6


def mppi_cost(X, U, ref_seq, collision_fn, w_pos=10.0, w_vel=0.5, w_u=0.05, w_terminal=0.01):
    """
    ...
    for t in range(N):
        ...
        cost += obstacle_penalty(p[0:2], collision_fn)

    # terminal cost
    eT = X[-1,0:3] - ref_seq[-1]
    cost += w_terminal * (eT @ eT)
    # return cost/N
    return cost
