...
class BaseAviary(gym.Env):
    ...
    def _create_walls(self):
        '''
        gets layout locations all walls and create a multibody with all of them, simulating a full floorplan
        
        :param self: pybullet enviroment
        '''

        walls = getLayout(random_obstacles="none",bounding_walls=True)

        for wall in walls:
            size_x = (wall.xmax - wall.xmin) / 2
            size_y = (wall.ymax - wall.ymin) / 2
            size_z = (wall.zmax - wall.zmin) / 2
            center_x = (wall.xmin + wall.xmax) / 2
            center_y = (wall.ymin + wall.ymax) / 2
            center_z = (wall.zmin + wall.zmax) / 2
            ...
            vis = p.createVisualShape(
                p.GEOM_BOX,
                halfExtents=[size_x, size_y, center_z],
                rgbaColor=wall.rgbaColor,
                physicsClientId=self.CLIENT
            )
